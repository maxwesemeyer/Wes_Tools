...
def clahe_nchannels(array):
    ...
    for i in range(array.shape[2]):
        array_squeezed = array[:, :, i].squeeze()
        new_array[:, :, i] = exposure.equalize_adapthist(array_squeezed, clip_limit=0.1,
...
class segmentation:
    ...
    def __init__(self, beta_coef=1, beta_jump=0.1, n_band=50, custom_subsetter=range(0, 80), MMU=0.05, PCA=True,
                 into_pca='all', n_class=4, iterations=70, filter_='bilateral'):
    ...
    def prepare_data(self, raster_l, vector_geom):
        ...
        if self.filter_:
            with rasterio.open(raster_l) as src:
                ...
                if self.filter_ == 'bilateral':
                    im = np.float32(im)
                    im = bilateral_nchannels(im)
                elif self.filter_ == 'clahe':
    ...
    def segment_2(self, string_to_raster, vector_geom, data_path_output, indexo=np.random.randint(0, 100000)):
        ...
        field_counter = "{}{}{}{}{}{}{}".format(str(self.n_band), "_", str(self.beta_jump), "_", str(self.beta_coef),
                                                str(indexo), str(self.PCA))
        ...
        if MMU_fail:
            ...
            clf = bayseg.BaySeg(three_d_image, mino, beta_init=self.beta_coef, stencil="4p", normalize=False)
